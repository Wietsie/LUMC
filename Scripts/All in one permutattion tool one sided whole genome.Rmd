---
title: "All in one permutattion tool one sided whole genome"
...
heatmap[1,e] <- 0.5-(length(temp[temp>CNV_overview[[1]][e]])/200)
}  
...
heatmap[2,e] <- 0.5-(length(temp[temp>CNV_overview[[2]][e]])/200)
}  
...
heatmap[3,e] <- 0.5-(length(temp[temp>CNV_overview[[3]][e]])/200)
}  
...
heatmap[4,e] <- 0.5-(length(temp[temp>CNV_overview[[4]][e]])/200)
} 
```
...
```{r}
heatmap(heatmap[1:4,], Colv = NA, Rowv = NA, scale="none", col = cm.colors(6))
